...
import { pathExists, workspacePath } from "./fs.js";
...
export async function readResource(uri) {
    ...
    if (uri.startsWith(wsTreePrefix) && uri.endsWith("/tree")) {
        ...
        if (!(await pathExists(wsRoot)))
            throw new ToolError("NOT_FOUND", "workspace not found");
    ...
    if (uri.startsWith(jobPrefix) && uri.endsWith("/latest_status")) {
        ...
        if (matches.length > 1) {
            return {
                mimeType: "application/json",
                text: JSON.stringify({ ok: false, error: "ambiguous job_name across workspaces", matches: matches.map((m) => path.relative(cfg.workspaceRoot, m).replaceAll("\\", "/")) }, null, 2),
            };
        }
    ...
    if (uri.startsWith(campaignPrefix) && uri.endsWith("/latest_status")) {
        ...
        const renderCampaignStatus = async (workspace, campaignName) => {
        ...
        if (campaignNameRaw.includes("/")) {
            ...
            if (!ws || !cn)
                throw new ToolError("INVALID_ARGUMENT", "campaign_name must be 'campaign' or 'workspace/campaign'");
