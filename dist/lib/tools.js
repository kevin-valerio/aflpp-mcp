...
import { requireObject, requireOptionalBoolean, requireOptionalNumber, requireOptionalString, requireString, requireStringArray, validateName, } from "./validate.js";
const TOOL_SPECS = [];
...
async function readJsonFileIfExists(p) {
    try {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (await pathExists(candidate))
                return path.join(jobOutDir, entry.name);
        }
    }
    catch {
        return null;
...
async function countFindings(dirPath) {
    if (!(await pathExists(dirPath)))
        return 0;
    const entries = await fs.readdir(dirPath, { withFileTypes: true });
    return entries.filter((e) => e.isFile() && e.name !== "README.txt").length;
}
async function listFindings(dirPath, type, relBase) {
    if (!(await pathExists(dirPath)))
        return [];
    const entries = await fs.readdir(dirPath, { withFileTypes: true });
    const out = [];
    for (const entry of entries) {
        if (!entry.isFile() || entry.name === "README.txt")
            continue;
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (!spec)
            return err("aflpp.help", "NOT_FOUND", `Unknown tool '${toolName}'`);
...
registerTool({
    ...
    description: "Create a workspace under workspaces/<name> with standard subdirectories: in,out,targets,build,logs,dicts,repros,reports.",
    inputSchema: globalInputSchema({
        name: { type: "string" },
    }, ["name"]),
    handler: async (args) => {
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const checks = async (rel) => pathExists(path.join(projectPath, rel));
...
registerTool({
    ...
    description: "Run a constrained build command with AFL++ compiler wrappers and copy the resulting artifact into the workspace targets/ directory.",
    inputSchema: globalInputSchema({
...
registerTool({
    ...
    description: "Build a CMPLOG-instrumented variant by setting AFL_LLVM_CMPLOG=1 during compilation (LLVM mode) and copy the artifact into the workspace targets/ directory.",
    inputSchema: globalInputSchema({
        workspace: { type: "string" },
        target_name: { type: "string" },
        project_path: { type: "string" },
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (!(await pathExists(corpusDir)))
            throw new ToolError("NOT_FOUND", "corpus not found");
        let files = 0;
        let bytes = 0;
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (path.relative(cfg.workspaceRoot, dictAbs).startsWith(".."))
            throw new ToolError("PATH_OUTSIDE_ROOT", "dict_path must be within workspace root");
        if (!(await pathExists(dictAbs)))
            throw new ToolError("NOT_FOUND", "dict_path not found");
        ...
        const dicts = Array.isArray(existing.dictionary_paths) ? existing.dictionary_paths : [];
        const destRel = path.relative(cfg.workspaceRoot, dest).replaceAll("\\", "/");
        const next = Array.from(new Set(dicts
...
registerTool({
    name: "aflpp.dry_run",
    description: "Run a short harness validation directly against the target (not afl-fuzz): checks input mode, stability, and basic performance signals.",
    inputSchema: globalInputSchema({
        workspace: { type: "string" },
    ...
    handler: async (args) => {
        ...
        const avgMs = results.reduce((sum, r) => sum + (typeof r.durationMs === "number" ? r.durationMs : 0), 0) / results.length;
        const verdict = {
        ...
        const nextSteps = [];
        if (timedOutAny)
            nextSteps.push("Increase exec timeout (-t) and/or fix hangs/timeouts in the harness.");
        if (!stableExit)
            nextSteps.push("Ensure the target exits consistently for seed inputs (remove nondeterminism/state).");
        if (avgMs > 50)
            nextSteps.push("Target is slow; consider persistent mode and minimizing initialization work.");
        if (nextSteps.length === 0)
            nextSteps.push("Proceed to aflpp.start_fuzz and poll aflpp.status regularly.");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (path.relative(cfg.workspaceRoot, testcaseAbs).startsWith(".."))
            throw new ToolError("PATH_OUTSIDE_ROOT", "testcase_path must be within workspace root");
        if (!(await pathExists(testcaseAbs)))
            throw new ToolError("NOT_FOUND", "testcase_path not found");
        const timeoutMs = requireOptionalNumber(args.timeout_ms, "timeout_ms") ?? 1000;
    ...
    handler: async (args) => {
        ...
        if (resume) {
            if (!(await pathExists(outDir)))
                throw new ToolError("NOT_FOUND", "resume=true but output directory does not exist");
        }
        else {
        ...
        const configDicts = Array.isArray(jobConfig.dictionary_paths) ? jobConfig.dictionary_paths : [];
        const mergedDicts = Array.from(new Set(configDicts
            .filter((d) => typeof d === "string")
            .concat(explicitDicts)
            .slice(-4)));
        const argv = [aflBin("afl-fuzz")];
        argv.push("-i", resume ? "-" : inputDir);
        argv.push("-o", outDir);
        argv.push("-T", jobName);
        if (execTimeoutMs !== undefined)
            argv.push("-t", String(execTimeoutMs));
        if (memLimitMb !== undefined)
            argv.push("-m", String(memLimitMb));
        if (seed !== undefined)
            argv.push("-s", String(seed));
        if (fuzzSeconds !== undefined)
            argv.push("-V", String(fuzzSeconds));
        for (const dp of mergedDicts)
            argv.push("-x", path.resolve(cfg.workspaceRoot, dp));
        if (cmplogPathAbs)
            argv.push("-c", cmplogPathAbs);
        argv.push("--", ...targetCmd);
        const env = {
            ...process.env,
        ...
        if (!resume)
            await fs.writeFile(jobLogPath, "", "utf8");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (resume) {
            if (!(await pathExists(outDir)))
                throw new ToolError("NOT_FOUND", "options.resume=true but output directory does not exist");
        }
        else {
            if (await pathExists(outDir)) {
        ...
        try {
            ...
            for (const inst of started) {
                const pid = typeof inst.pid === "number" ? inst.pid : null;
                if (!pid)
                    continue;
                try {
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (!meta || typeof meta.pid !== "number")
            throw new ToolError("NOT_FOUND", "job metadata not found");
        const pid = meta.pid;
        let stopped = false;
        try {
            process.kill(-pid, "SIGTERM");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (!meta || typeof meta !== "object")
            throw new ToolError("NOT_FOUND", "campaign metadata not found");
        ...
        for (const inst of instances) {
            ...
            if (typeof obj.pid !== "number")
                continue;
            const name = typeof obj.instance_name === "string" ? obj.instance_name : "unknown";
            pids.push({ instance_name: name, pid: obj.pid });
        }
        if (pids.length === 0)
            throw new ToolError("NOT_FOUND", "no instance PIDs recorded for this campaign");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const outDir = workspacePath(cfg.workspaceRoot, workspace, "out", jobName);
        if (!(await pathExists(outDir)))
            throw new ToolError("NOT_FOUND", "job output directory not found");
        const instanceDir = await findInstanceDir(outDir);
        if (!instanceDir)
            throw new ToolError("NOT_FOUND", "could not locate fuzzer_stats for this job yet");
        ...
        if (last && typeof last === "object") {
            ...
            deltas.counts = {
                queue: queueCount - (typeof lastCounts.queue === "number" ? lastCounts.queue : 0),
                crashes: crashCount - (typeof lastCounts.crashes === "number" ? lastCounts.crashes : 0),
                hangs: hangCount - (typeof lastCounts.hangs === "number" ? lastCounts.hangs : 0),
            };
        }
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const outDir = workspacePath(cfg.workspaceRoot, workspace, "out", campaignName);
        if (!(await pathExists(outDir)))
            throw new ToolError("NOT_FOUND", "campaign output directory not found");
        const metaPath = workspacePath(cfg.workspaceRoot, workspace, "reports", "campaigns", `${campaignName}.json`);
        const meta = await readJsonFileIfExists(metaPath);
        const metaInstances = meta && Array.isArray(meta.instances) ? meta.instances : [];
        const pidByInstance = new Map();
        for (const inst of metaInstances) {
            if (!inst || typeof inst !== "object")
                continue;
            const obj = inst;
            if (typeof obj.instance_name !== "string" || typeof obj.pid !== "number")
                continue;
            pidByInstance.set(obj.instance_name, obj.pid);
        }
        ...
        for (const name of instanceDirs) {
            ...
            if (Number.isFinite(execsDone))
                totalExecsDone += execsDone;
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const env = { ...process.env, AFL_PATH: cfg.aflppDir };
        ...
        if (instanceNames.length === 0)
            throw new ToolError("NOT_FOUND", "no instances with fuzzer_stats found for this campaign yet");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const instanceDir = await findInstanceDir(outDir);
        if (!instanceDir)
            throw new ToolError("NOT_FOUND", "could not locate fuzzer output structure yet");
...
registerTool({
    ...
    description: "Reproduce a finding by running the target command directly with the testcase; captures stdout/stderr and writes a repro bundle under repros/.",
    inputSchema: globalInputSchema({
        workspace: { type: "string" },
        job_name: { type: "string" },
    ...
    handler: async (args) => {
        ...
        for (const sub of ["crashes", "hangs"]) {
            ...
            for (const entry of entries) {
                ...
                if (id === findingId)
                    candidates.push({ type: sub === "crashes" ? "crash" : "hang", abs, rel });
            }
        }
        if (candidates.length === 0)
            throw new ToolError("NOT_FOUND", "finding_id not found");
        ...
        const sanitizerHint = stderrLower.includes("addresssanitizer") ? "asan" : stderrLower.includes("undefinedbehavior") ? "ubsan" : null;
...
registerTool({
    ...
    handler: async (args) => {
        ...
        let repro = existingRepro;
        if (!repro) {
            const metaArgvRaw = Array.isArray(jobMeta.argv) ? jobMeta.argv : [];
            const metaArgv = metaArgvRaw.filter((v) => typeof v === "string");
            const sep = metaArgv.indexOf("--");
            if (sep === -1)
                throw new ToolError("NOT_FOUND", "job metadata missing argv separator '--'");
        ...
        const sanitizer = typeof repro.sanitizer === "string" ? repro.sanitizer : null;
        const extractFrames = (stderrText) => {
            const frames = [];
        ...
        const targetCmdFromRepro = Array.isArray(repro.target_cmd) ? repro.target_cmd : [];
        const targetCmd = targetCmdFromRepro.filter((v) => typeof v === "string");
        const usesAtAt = targetCmd.some((a) => a.includes("@@"));
        const inputMode = usesAtAt ? "@@" : "stdin";
        const relevantEnv = {};
        ...
        const report = {
            ...
            repro: {
                ...
                target_cmd: targetCmd,
                argv: Array.isArray(repro.argv) ? repro.argv : null,
                cwd: cfg.workspaceRoot,
                env: relevantEnv,
        ...
        if (signal)
            mdLines.push(`- Signal: \`${signal}\``);
        if (exitCode !== null)
            mdLines.push(`- Exit code: \`${exitCode}\``);
        if (sanitizer)
            mdLines.push(`- Sanitizer: \`${sanitizer}\``);
        mdLines.push("");
...
registerTool({
    ...
    handler: async (args) => {
        ...
        if (!(await pathExists(inputDir)))
            throw new ToolError("NOT_FOUND", "corpus not found");
        const outCorpusName = validateName(requireOptionalString(args.output_corpus_name, "output_corpus_name") ?? `${corpusName}_cmin`, "output_corpus_name");
        const outputDir = workspacePath(cfg.workspaceRoot, workspace, "in", outCorpusName);
        if (await pathExists(outputDir))
            throw new ToolError("ALREADY_EXISTS", "output corpus already exists");
        await ensureDir(outputDir);
        const execTimeoutMs = requireOptionalNumber(args.timeout_ms, "timeout_ms");
        const memLimitMb = requireOptionalNumber(args.mem_limit_mb, "mem_limit_mb");
        const toolTimeoutMs = requireOptionalNumber(args.tool_timeout_ms, "tool_timeout_ms") ?? 10 * 60_000;
        const argv = [aflBin("afl-cmin"), "-i", inputDir, "-o", outputDir];
        if (execTimeoutMs !== undefined)
            argv.push("-t", String(execTimeoutMs));
        if (memLimitMb !== undefined)
            argv.push("-m", String(memLimitMb));
...
registerTool({
    ...
    handler: async (args) => {
        ...
        const argv = [aflBin("afl-tmin"), "-i", testcaseAbs, "-o", outPath];
        if (execTimeoutMs !== undefined)
            argv.push("-t", String(execTimeoutMs));
        if (memLimitMb !== undefined)
            argv.push("-m", String(memLimitMb));
