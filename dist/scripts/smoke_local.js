...
async function main() {
    ...
    await fs.writeFile(path.join(projectDir, "Makefile"), ["all: test-instr", "", "test-instr: test-instr.c", "\t$(CC) -o test-instr test-instr.c", "", "clean:", "\trm -f test-instr", ""].join("\n"), "utf8");
    ...
    const storedPath = build.data.artifact?.stored_path;
    if (!storedPath)
        throw new Error("build_instrumented did not return artifact.stored_path");
